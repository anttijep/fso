    ...
    describe("When logged in", function () {
      ...
      describe("blogs exists", function () {
        ...
        it("A blog can be liked", function () {
          cy.get("#link" + blogsindb[0].id)
            .click()
            .then(() => {
              cy.get("#" + blogsindb[0].id + "like")
                .click()
                .then(() => {
                  cy.get("#" + blogsindb[0].id + "like")
                    .parent()
                    .contains(String(blogsindb[0].likes + 1));
                });
            });
        });
        ...
        it("A blog can be deleted", function () {
          cy.get("#link" + blogsindb[0].id)
            .click()
            .then(() => {
              cy.get("#" + blogsindb[0].id + "remove")
                .click()
                .then(() => {
                  cy.get("#link" + blogsindb[0].id).should("not.exist");
                });
            });
        });
                ...
                .then(() => {
                  cy.visit("http://localhost:3000").then(() => {
                    cy.get(".all-blogs")
                      ...
                      .should(
                        "contain",
                        `${blogsindb[0].title} ${blogsindb[0].author}`
                      )
                      .then(() => {
                        cy.get("#link" + blogsindb[1].id)
                          .click()
                          .then(() => {
                            cy.get("#" + blogsindb[1].id + "like")
                              .click()
                              .then(() => {
                                cy.get("#" + blogsindb[1].id + "like")
                                  .click()
                                    ...
                                    cy.visit("http://localhost:3000").then(
                                      () => {
                                        cy.get(".all-blogs")
                                          ...
                                          .should(
                                            "contain",
                                            `${blogsindb[1].title} ${blogsindb[1].author}`
                                          );
                                      }
                                    );
                                  });
